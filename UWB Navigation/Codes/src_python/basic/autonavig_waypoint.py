    ...
    while(True):
        ...
        try:
            ...
            while(mag > 0.5):
                try:
                    line = ser.readline()
                    radialPos = [float(z) for i in line.split(',')]
                    radialPos = [radialPos[z]*alpha + prevRadialPos[i]*(1-alpha) for z in range(0, 3)]
                    prevRadialPos = radialPos
                    ...
                    eX = position[0] - datx
                    eY = position[1] - daty
                    pSteer, eX_prev, uX_prev = calcPid(eX, eX_prev, uX_prev, k, Xconsts)
                    rSteer, eY_prev, uY_prev = calcPid(eY, eY_prev, uY_prev, k, Yconsts)
                    k = k+1
                    # print X,Y
