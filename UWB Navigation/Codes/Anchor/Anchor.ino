...
// connection pins, modify accordingly for arduino pro micro and STM32 
...
int idx = 0;
uint8_t anchorId = 1; // Id of this anchor
uint8_t tagId = 0; // Id of the tag 
...
void loop() {
  ...
  if (sentAck) {
    sentAck = false;
    byte msgId = data[0];
    if ((data[2] == tagId) && (data[1] == anchorId)) {
      if (msgId == POLL_ACK) {
  ...
  if (receivedAck) {
    ...
    if ((data[2] == tagId) && (data[1] == anchorId)) {
      ...
      if (msgId == POLL) {
        ...
        expectedMsgId = POLL;
        if (!protocolFailed) {
          timePollSent.setTimestamp(data + 3);
          timePollAckReceived.setTimestamp(data + 8);
          timeRangeSent.setTimestamp(data + 13);
